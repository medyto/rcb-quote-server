...
const FOAM_PRICES = { '0':0, 'NO':0, '1':3.35, '2':4.44, '2.5':6.15, '3':7.80, 'MEM':12, 'LTX':15, 'DFT':14.73 };
...
function calcLine(qty, W, L, T, density, fabricPrice, hasBack, extras) {
  ...
  const noFoam = density === 'NO';
  const noFabric = fabricPrice === 0;
  const foamPrice = noFoam ? 0 : (FOAM_PRICES[String(density)] || 0);
  const rawYards = noFabric ? 0 : (W + T) * (L * AC) / 1296;
  const yards = rawYards === 0 ? 0 : Math.ceil(rawYards * 2) / 2;
  const fabricCost = yards * fabricPrice;
  const foamBF = noFoam ? 0 : (W * L) / 144 * T;
  const foamCost = foamBF * foamPrice;
  const stripHrs = noFoam ? 0 : (T <= 4 ? 0.3 : 0.25);
  const sewHrs = noFabric ? 0 : (W + L) * 3 / 200;
  const foamHrs = noFoam ? 0 : (W + L) * 3 / 300;
  ...
  const foamLabel = noFoam ? 'no foam' : density + 'lb foam';
  const fabricLabel = noFabric ? 'no fabric' : '$' + fabricPrice + '/yd fabric';
  return {
    label: qty + ' x ' + W + '"x' + L + '"x' + T + '" (' + foamLabel + ', ' + fabricLabel + ')',
    yards: yards * qty,
...
function buildEmailBody(customerName, customerEmail, customerCompany, customerPhone, lines, totals, extrasLabel) {
...
app.post('/api/quote', async function(req, res) {
  const { customerName, customerEmail, customerCompany, customerPhone, cushions, extras } = req.body;
  ...
  const emailBody = buildEmailBody(customerName, email, customerCompany, customerPhone, lines, totals, extrasLabel);
  ...
  try {
    await sgMail.send({
      from: { name: 'RCB Quote Form', email: process.env.SENDGRID_FROM_EMAIL },
      to: COMPANY_EMAIL,
      subject: 'New Quote Request from ' + customerName + ' — RCB Upholstery',
      text: emailBody,
